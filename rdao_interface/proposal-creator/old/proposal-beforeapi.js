...
function setMenuCoordinates(element, position) {
  ...
  mentionMenu.style.left = `${
    coords.left + element.scrollLeft + staticLeftOffset
  }px`;
}
...
menuItemFn = (user, idx, selected) => {
  ...
  div.innerHTML = `
      <div class="mentionEl">
        ...
        <p style="padding-left:10px">${user}</p>
        ${
...
function onKeyDown(ev) {
    ...
    if (triggerIdx !== undefined) {
      switch (ev.key) {
        case "ArrowDown":
          active = Math.min(active + 1, profileList.length - 1);
          //renderMenu();
