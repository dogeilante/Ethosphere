 ...
 * @param {*} nodeId 
 */
function setNodeDataOLD(nodeId) {
  ...
  left_neighbors.unshift(dummyData[left_neighbors[0].neighbors[0]])
  left_neighbors.unshift(dummyData[left_neighbors[0].neighbors[0]])
  currNodeList = left_neighbors.concat(currNode, right_neighbors)

}
function isRight(node1, node2) {
  // Extract the HSL values
  const hue1 = node1.values[3];
  const hue2 = node2.values[3];
  const sat1 = node1.values[4];
  const sat2 = node2.values[4];
  const light1 = node1.values[5];
  const light2 = node2.values[5];
  ...
  if (hue1 < hue2) {
      ...
      // If hue values are equal, compare the saturation values
      if (sat1 < sat2) {
          return true;
      } else if (sat1 > sat2) {
          return false;
      } else {
          // If both hue and saturation values are equal, compare the lightness values
          if (light1 < light2) {
              return true;
          } else {
              return false;
...
async function setNodeData(nodeId) {
  let centerNode = await sifter.getNode(nodeId);
  let left_neighbors = [];
  let right_neighbors = [];
  let queue = [centerNode];
  center_id = centerNode.id
  let qHistory = {center_id: centerNode};

  // Function to check if a node is already in the neighbors list
  const isNodeInNeighbors = (node, neighbors) => {
    return neighbors.some(neighbor => neighbor.id === node.id);
  };

  while ((left_neighbors.length < 4 || right_neighbors.length < 4) && queue.length !== 0) {
    let currNode = queue.shift();
    let neighbors = await sifter.getBranch(currNode.id);

    for (let i = 0; i < neighbors.length-1; i++) {
      let neighbor = neighbors[i];
      if (!qHistory[neighbor.id]) {
        queue.push(neighbor)
        qHistory[neighbor.id] = neighbor
        isRightNeighbor = isRight(centerNode, neighbor)
        if (isRightNeighbor && right_neighbors.length < 4) {
          right_neighbors.push(neighbor);
        } else if (!isRightNeighbor && left_neighbors.length < 4) {
          left_neighbors.push(neighbor);
        }
      }
      
      // Add the neighbor to the queue for further exploration if not already processed
      /*let isProcessed = isNodeInNeighbors(neighbor, left_neighbors) || isNodeInNeighbors(neighbor, right_neighbors) || neighbor == currNode
      if (!isProcessed && !qHistory.includes(currNode)) {
        queue.push(neighbor);
        qHistory.push(currNode);
        console.log(qHistory);
      }
      isRightNeighbor = isRight(centerNode, neighbor)
      if (isRightNeighbor && !isProcessed && right_neighbors.length < 4) {
        right_neighbors.push(neighbor);
      } else if (!isRightNeighbor && !isProcessed && left_neighbors.length < 4) {
        left_neighbors.push(neighbor);
      } else {
  ...
  // Ensure centerNode is in the middle of the result array
  //let neighbors = [...left_neighbors, centerNode, ...right_neighbors];
  left_neighbors.sort((a, b) => {
    if (isRight(centerNode, a) && !isRight(centerNode, b)) return -1;
    if (!isRight(centerNode, a) && isRight(centerNode, b)) return 1;
    return 0;
  });

  right_neighbors.sort((a, b) => {
    if (isRight(centerNode, a) && !isRight(centerNode, b)) return -1;
    if (!isRight(centerNode, a) && isRight(centerNode, b)) return 1;
    return 0;
  });
  // Get up to 4 neighbors from each side, padding with null if necessary
  const leftFour = left_neighbors.slice(0, 4);
  const rightFour = right_neighbors.slice(0, 4);

  while (leftFour.length < 4) {
      leftFour.unshift(null);
  }

  while (rightFour.length < 4) {
      rightFour.push(null);
  }
  
  currNodeList = [...leftFour, centerNode, ...rightFour]
  console.log(currNodeList);
}

function createNodeBoxes() {

  for (let i = 0; i < currNodeList.length; i++) {
      const currNode = currNodeList[i]
      const box = nodeList[i]
      box.id = `box${i}`;
      
      if (i === 4) { // the center will ALWAYS be the 
        box.innerHTML = `<h2>${currNode.id}</h2>`;
        currNode.values.forEach(val => {
        ...
        box.innerHTML = `<h2>${currNode.id}</h2>${currNode.values.join('<br>')}`;
      ...
      rectangle.style.background = `rgb(${parseInt(currNode.values[0])}, ${parseInt(currNode.values[1])}, ${parseInt(currNode.values[2])})`;
...
function updateCurrentNodeData() {
  ...
  currNodeList[CENTER_NODE].values.forEach(item => {
...
function addToHistory(nodeId) {
  ...
  if (!historyContainsVal && currentNodeVal != '') {
    ...
    historyDiv.textContent = `Node ${nodeId}: ${currentNodeVal}`;

    historyDiv.addEventListener('click', async () => {
      await navigateToNode(nodeId)
      backToThePast(nodeId)
    });
    historyList.appendChild(historyDiv);
    historyList.scrollTop = historyList.scrollHeight;
    siftHistory.push({"id": nodeId, "values": currNodeList, "historyDiv": historyDiv})
  }
  ...
  if (currentNodeVal != '') {
    ...
    if (identifier == 'r') {
      currentNodeVal = currNodeList[4].values[0];
    } else if (identifier == 'g'){
      currentNodeVal = currNodeList[4].values[1]
    } else {
      currentNodeVal = currNodeList[4].values[2]
    }
  }
  
}
/**
 * This function is equivalent to REMOVING from history
 */
function backToThePast(nodeId) {

}

async function navigateToNode(nodeId) {
  currentNodeId = nodeId;
  await setNodeData(nodeId);
  updateCurrentNodeData(nodeId);
  updateNodeData(nodeId);
  createNodeBoxes();
  // addToHistory(nodeId); We want to add to history ONLY when a user clicks on a value
}

leftArrow.addEventListener('click', async () => {
  await navigateToNode(currNodeList[3].id);
});

rightArrow.addEventListener('click', async () => {
  await navigateToNode(currNodeList[5].id);
});
...
function generateRGBCombinations() {
  const step = 85;
  const values = [0, step, 2 * step, 3 * step];
  const combinations = [];
...
trackpad.addEventListener('mousemove', function(event) {
    ...
    velocity = (x - middleX) > 0 ? 1 + colorRatio * 14 : -(1 + colorRatio * 14); // velocity ranges from 1 to 15
});
...
function startInfiniteLoop() {
    async function loop() {
        ...
        setTimeout(loop, 1000 / speed); // Loop frequency based on speed
        // move right
        if (velocity > 0) {
          await navigateToNode(currNodeList[5].id)
        // move left
